...
main( int argc, char *argv[] )
{
  ...
  if(strlen(row[7]) && strlen(row[8])){
    ...
    if( ! (strlen(row[0]) && strlen(row[1]) && strlen(row[2])))
      {
    ...
    rval = Ganglia_metric_set( gmetric, row[0], row[1],
             row[2], row[3], cstr_to_slope(row[4]),
  ...
  if(strlen(row[7]))
    //Ganglia_metadata_add(gmetric, SPOOF_HOST, args_info.spoof_arg);
      Ganglia_metadata_add(gmetric, SPOOF_HOST, row[7]);
